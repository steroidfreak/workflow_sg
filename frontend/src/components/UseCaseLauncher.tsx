...
const UseCaseLauncher: React.FC = () => {
  ...
  return (
    <div className="fixed top-6 inset-x-0 z-50 flex flex-col items-center gap-3">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            key="use-case-panel"
            ...
            className="flex w-[320px] flex-col rounded-3xl border border-slate-200 bg-white p-5 text-sm text-slate-800 shadow-2xl shadow-slate-200/70 backdrop-blur dark:border-white/10 dark:bg-slate-950/95 dark:text-slate-100"
          >
      ...
      <button
        ...
        className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-500/30 transition hover:bg-slate-800 focus:outline-none focus:ring focus:ring-slate-500/30 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
      >
